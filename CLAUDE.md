...
- **The 4 scores (Bounce/Power/Control/Form) = BUILT**, in
  ...
  **0–100 bands are documented coaching heuristics, not validated norms** —
  ...
  Still TODO here: written coaching text off the scores, and knee-angle
  metrics (the knee landmarks are captured but nothing reads them yet).
...
- [x] **Analyze tab v1** — record/import video → trim → automatic pose
      ...
      can't support it. Still TODO: written coaching off the scores, knee-
      angle metrics, potential projection on the result screen, share,
      camera/library Info.plist permissions (see above).
